...
| 安排 | 登记今天已发生或接下来的事项、起止时间、时间去向、可选体感与备注。 | 关闭页面，零写入。 | 一条 `time_use` 类型信号卡；结构化时间保存在 `raw_payload_json`。 |
...
未确认／legacy 内容最多作为轻背景。当前版本的外部提示只允许包含睡眠恢复或活动恢复等 Health 抽象值，不能包含原始健康样本或精确健康时间线。外部提示与用户记录冲突时，以用户明确记录为准；缺少能量证据时保持 unknown，不用外部提示补出结论，也不能只靠外部提示提高建议强度。用户主动登记的 `time_use` 信号可以把场景和可选体感带入 Energy Budget，但不能只凭时长推断耗能。系统 Calendar 日程密度能力仍只作为未来 Target 保留在底层，不进入当前用户入口、候选上下文或来源哈希。
...
| 信号库 | Today 记录栏 | 提供固定、隐私安全的信号卡参考。 | 已实现的辅助功能。 |
...
本轮已经进入已实现基线、但仍需下一次真机回归的能力包括：当天／当周三信号门槛、两个复数候选页、0..N 多选采纳、同周多个正式实验、Today 的 3+3 投影上限、两种对象各自真实的 `X/7`、来源变化后的即时失效与原位更新，以及 Today 内部 `time_use` 安排信号的登记、时间线显示和 Energy Budget 读取。
